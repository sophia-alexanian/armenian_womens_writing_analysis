...
categories = {
    "Geographic Regions": ["Mediterranean", "sevan", "mediterranean", "Europe", "Middle East", "karabakh", "nagornokarabakh", "Caucasus", "caucasus", "nagorno-karabakh", "west", "crimea"],
    "Political Terms": ["democracy", "election", "policy", "security", "minister", "normalization", "territorial", "ministry", "corruption", "rights", "civic", "state", "government", "vote", "independence", "revolution", "protest", "justice", "marches", "free", "freedom", "independent", "independence"],
    "Politicians": ["Donald Trump", "Trump", "trump", "biden", "putin", "Joe Biden", "Biden", "Macron", "Emmanuel Macron", "Nikol Pashinyan", "Pashinyan", "pashinyan", "nikol", "macron", "Ilham Aliyev", "Putin", "aliyev", "ilham", "Kocharyan", "erdogan", "sargsyan"],
    "Alliances": ["EU", "EEU", "CSTO", "NATO"],
    "Organizations": ["ARS", "AGBU", "ARF", "AYF", "ANCA", "homenetmen", "hamazkayin", "community", "council", "members", "chapters"],
    ...
    "Cities": ["gyumri", "martuni", "brussels", "boston", "damascus", "chicago", "ani", "van", "beirut", "aleppo", "yerevan", "baku", "paris", "berlin", "moscow", "toronto", "watertown", "tel aviv", "jerusalem", "haifa", "nyc", "los angeles", "glendale", "francisco"],
    "Countries": ["armenia", "spain", "belgium", "poland", "lebanon", "ukraine", "artsakh", "syria", "lebanon", "syria", "turkey", "azerbaijan", "Azerbaijan", "canada", "australia", "israel", "palestine", "germany", "france", "russia", "china", "usa", "america", "artsakh", "georgia", "sweden", "greece", "abkhazia", "kosovo"],
    ...
    "Culture": ["poetry", "publication", "journalism", "art", "exhibits", "heritage", "cultural", "books", "musuem", "painting", "sculpture", "culture", "literature", "dance", "music", "song", "book", "comedy", "performance", "humor"],
...
def categorize_keywords(keywords, category_vectors, similarity_threshold=SIMILARITY_THRESHOLD):
    ...
    for keyword in keywords:
        ...
        matched = False
        for cat, words in categories.items():
            if keyword.lower() in map(str.lower, words):  # Case-insensitive match
                categorized[cat].append(keyword)
                matched = True
                break

        if not matched:  # Proceed with semantic similarity only if no exact match
            keyword_vector = model.encode(keyword, convert_to_tensor=True)
            similarities = {
                cat: cosine_similarity(
                    keyword_vector.cpu().numpy().reshape(1, -1),
                    category_vector.cpu().numpy().reshape(1, -1)
                )[0][0]
